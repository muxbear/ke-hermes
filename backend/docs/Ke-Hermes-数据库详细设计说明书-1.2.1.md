...
| 数据类型 | 存储 | 原因 |
|----------|------|------|
| 用户信息、OAuth 绑定、登录日志 | SQLite (aiosqlite) | 需持久化、支持复杂查询、关联查询 |
| 验证码、限流计数、OAuth state | Redis/Memory | 短期有效、自动过期、高频读写 |
| Agent 对话检查点 | SQLite / PostgreSQL（可选） | LangGraph 框架管理，与业务库隔离 |
| Agent 文件操作 | FilesystemBackend (workspace/) | 虚拟文件系统，支持回滚 |
...
| 数据库 | `./db/langgraph_checkpoints.db` | PostgreSQL 数据库（可配置） |
| 配置项 | `CHECKPOINT_DB_PATH`（默认 `./db/langgraph_checkpoints.db`） | `CHECKPOINT_DB_URL`（默认 `postgresql://127.0.0.1:5432/ke_hermes`） |
...
| FilesystemBackend | `workspace/` 目录（虚拟模式） | Agent 文件读写、代码执行输出的虚拟文件系统 |
...
CHECKPOINT_DB_PATH=./db/langgraph_checkpoints.db   # SQLite 检查点数据库文件路径
...
- FilesystemBackend 使用 `workspace/` 目录，virtual_mode 使文件操作可回滚
...
| `backend/src/agent/graph.py` | Agent 图定义 + LangGraph 检查点双后端初始化 + 生命周期管理 |
...
| `backend/workspace/` | Agent 虚拟文件系统工作目录（FilesystemBackend） |
...
| 虚拟文件系统 | FilesystemBackend | DeepAgents 提供的 Agent 文件操作后端，virtual_mode 下可回滚 |
