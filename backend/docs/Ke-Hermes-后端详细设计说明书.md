...
本文档为 Ke-Hermes 后端的详细设计说明书 v1.4.0，对照实际代码实现编写。文档覆盖：

- **已实现模块**：智能体框架（含子智能体系统 + 互联网搜索工具 + CompositeBackend 文件后端）、Chat API（普通对话 + SSE 流式 + thread_id 上下文管理）、Conversation API（对话历史 CRUD）、MCP 广场 API（工具列表/详情/安装/卸载 + 种子数据）、Skills 技能管理 API（CRUD + 上传校验 + 批量删除）、认证授权、验证码、OAuth 第三方登录、短信服务、邮箱验证码
- **基础设施**：KeyValueStore 存储抽象层（Redis + 内存降级）、RSA + JWT + bcrypt 安全体系、SQLAlchemy 异步 ORM（SQLite + PostgreSQL 双后端，7 个 ORM 模型）、LangGraph 检查点双后端（SQLite + PostgreSQL）+ LangGraph Store（Memory + PostgreSQL）
...
3. `init_graph()` → 根据 `CHECKPOINT_BACKEND` 创建检查点后端 + LangGraph Store + LLM + 子智能体 + CompositeBackend
...
共 **30 个接口**（v1.3.0 为 21 个，v1.4.0 新增 1 个 Email + 4 个 MCP + 9 个 Skills 接口）。
...
启动后访问 `http://127.0.0.1:8000/docs`，FastAPI 自动生成交互式 Swagger UI 文档。9 个功能模块共 30 个接口均自动注册。
...
### 13.3 从 v1.0.0 到 v1.4.0 的变更总结


| 项目       | v1.0.0 (初始)          | v1.4.0 (当前)                                                                                     |
| -------- | -------------------- | ----------------------------------------------------------------------------------------------- |
| API 模块   | 1 (agent)            | 9 (agent + auth + captcha + oauth + sms + email + conversation + mcp + skill)                   |
| 数据模型     | 0                    | 7 (User + UserOAuth + LoginRecord + Conversation + Skill + McpTool + McpInstallation)           |
...
| 安全       | 无                    | RSA + JWT + bcrypt + 登录锁定 + JWT 鉴权（Chat + Conversation + MCP + Skill API）                    |
...
| 外部依赖     | DeepSeek + DashScope | + bcrypt + cryptography + httpx + Pillow + redis(可选) + tavily + psycopg(可选) + python-multipart |
| 环境变量     | 8                    | 39+                                                                                             |
| API 接口   | 2                    | 30                                                                                              |
...
> 本文档 v1.4.0 基于实际代码实现更新。v1.4.0 重点新增了 MCP 广场模块（工具列表/详情/安装/卸载 + 12 个预置种子工具）、Skills 技能管理模块（上传校验/CURD/批量删除/文件系统安装）、Email 邮箱验证码模块；数据模型从 4 个扩展到 7 个（新增 Skill/McpTool/McpInstallation）；API 接口从 21 个扩展到 30 个；API 子模块从 6 个扩展到 9 个。后续版本应优先实现模型列表 API 和通知系统。
