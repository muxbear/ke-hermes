...
本文档为 Ke-Hermes 后端的详细设计说明书 v1.2.2，对照实际代码实现编写。文档覆盖：

- **已实现模块**：智能体框架（含互联网搜索工具 + 文件系统后端）、Chat API（普通对话 + SSE 流式 + thread_id 上下文管理）、认证授权、验证码、OAuth 第三方登录、短信服务
- **基础设施**：KeyValueStore 存储抽象层（Redis + 内存降级）、RSA + JWT + bcrypt 安全体系、SQLAlchemy 异步 ORM、LangGraph 检查点双后端（SQLite + PostgreSQL）
...
| 互联网搜索        | tavily-python                                    | —           | 智能体互联网搜索工具               |
...
│   │   ├── tools/
│   │   │   ├── __init__.py          # 工具列表导出
│   │   │   └── internet_search.py   # Tavily 互联网搜索工具
│   │   ├── utils/
│   │   │   └── __init__.py          # 辅助方法导出（预留）
│   │   └── graph.py                 # deepagents 智能体图定义（生命周期管理）
│   │
│   ├── api/
...
│
├── db/                              # SQLite 数据库文件目录
│   ├── ke-hermes.db                 # 开发环境 SQLite 业务数据库文件
│   └── langgraph_checkpoints.db     # LangGraph 检查点数据库文件（SQLite 后端）
│
├── workspace/                       # 智能体虚拟文件系统工作目录（FilesystemBackend）
│
├── tests/
...
async def lifespan(app: FastAPI):
    ...
    await init_graph()            # 2. 初始化 Agent（检查点后端 + LLM + 工具）
    store = await create_store()   # 3. 初始化 KeyValueStore（Redis 或内存降级）
    set_store(store)
    from core.security import _get_jwt_secret as init_jwt
    init_jwt()                    # 4. 预初始化 JWT secret
    yield
    await shutdown_graph()        # 5. 关闭检查点连接池（PostgreSQL）
...
2. `init_graph()` → 根据 `CHECKPOINT_BACKEND` 创建检查点后端 + LLM + 工具 + FilesystemBackend
3. `create_store()` → 尝试连接 Redis，失败则降级为 `MemoryStore`
4. `set_store(store)` → 注入到 FastAPI 依赖注入系统
5. `init_jwt()` → 读取或生成持久化 JWT 签名密钥
6. (关闭时) `shutdown_graph()` → 关闭 PostgreSQL 连接池
...
#### `config.py` — Settings 类（v1.2.2 扩展）
...
class Settings(BaseSettings):
    ...
    # ---- Checkpoint Database ----
    CHECKPOINT_BACKEND: str = os.getenv("CHECKPOINT_BACKEND", "sqlite")    # sqlite | postgres
...
- `CHECKPOINT_BACKEND` — LangGraph 检查点后端选择（`sqlite` / `postgres`）
- `CHECKPOINT_DB_PATH` — SQLite 检查点数据库文件路径
- `CHECKPOINT_DB_URL` — PostgreSQL 检查点数据库连接串
- 配置值通过 `os.getenv()` 显式读取，配合 `load_dotenv()` 在模块顶层调用
...
### 4.4 智能体图 — `agent/graph.py`（v1.2.2 重构）

**职责：** 管理 Agent 图的完整生命周期（初始化 → 运行 → 关闭），支持双检查点后端（SQLite / PostgreSQL），集成互联网搜索工具和虚拟文件系统后端。

#### 生命周期架构

v1.2.2 将 Agent 图从模块级全局变量重构为 `init_graph()` / `get_graph()` / `shutdown_graph()` 生命周期模式：
...
- SQLite 后端：数据库文件位于 `CHECKPOINT_DB_PATH`（默认 `./db/langgraph_checkpoints.db`）
- PostgreSQL 后端：需要 `psycopg` 驱动 + `psycopg_pool` 连接池，使用 `dict_row` 行工厂，启动时自动执行 `checkpointer.setup()` 创建检查点表
...
| 工具                | 来源                               | 说明                                  |
| ----------------- | -------------------------------- | ----------------------------------- |
| `internet_search` | `agent/tools/internet_search.py` | 基于 Tavily API 的互联网搜索工具，使智能体具备联网检索能力 |
...
- `**root_dir**`: 工作目录为 `backend/workspace/`
- `**virtual_mode=True**`: 使用虚拟文件系统模式——工具操作虚拟化，可回滚，不直接写入真实文件系统
- `FilesystemBackend` 由 `deepagents` 框架提供，使智能体具备文件读写、代码执行等能力
...
所有 API 层通过 `get_graph()` 获取 Agent 图实例，不再直接引用模块级变量。

### 4.5 Chat API — 对话与流式接口（v1.2.2 更新）
...
class ChatRequest(BaseModel):
    ...
    thread_id: str | None = None       # v1.2.2 新增：对话线程 ID

class ChatResponse(BaseModel):
    response: str
    thread_id: str                     # v1.2.2 新增：返回当前对话线程 ID
...
**调用流程：** ChatRequest → 生成/复用 thread_id (UUID7) → `get_graph().ainvoke()` → DeepSeek LLM → 取最后 AIMessage → ChatResponse (含 thread_id)
...
async def chat_stream(req: ChatRequest):
    ...
    async def event_generator():
        ...
        yield f"data: {json.dumps({'thread_id': thread_id})}\n\n"  # v1.2.2: 结束时返回 thread_id

    return StreamingResponse(event_generator(), media_type="text/event-stream")
```

**调用流程：** ChatRequest → 生成/复用 thread_id → `get_graph().astream_events(v2)` → 过滤 `on_chat_model_stream` → 逐 token SSE 推送 → 最后推送 thread_id

#### thread_id 上下文管理（v1.2.2 新增）
...
UUID7 按时间戳排序，天然适合检查点数据库索引。LangGraph 通过 `configurable.thread_id` 自动管理检查点的读写——同一 thread_id 的多次请求自动恢复历史消息并追加新检查点。
...
| 接口                          | 方法   | 模块      | 后端实现  | 前端调用                            |
| --------------------------- | ---- | ------- | ----- | ------------------------------- |
| `/api/chat`                 | POST | agent   | ✅ 已实现 | sendChatRequest (含 thread_id)   |
| `/api/chat/stream`          | POST | agent   | ✅ 已实现 | sendStreamRequest (含 thread_id) |
...
thread_id = req.thread_id or uuid7()
    ↓
get_graph().ainvoke({"messages": [HumanMessage(...)]}, config={"configurable": {"thread_id": ...}})
    ↓
LangGraph 从检查点恢复上下文（续接对话）→ deepagents 执行智能体图 → ChatOpenAI(DeepSeek) 生成回复
    ↓
...
thread_id = req.thread_id or uuid7()
    ↓
get_graph().astream_events({"messages": [HumanMessage(...)]}, config=..., version="v2")
    ↓
LangGraph 从检查点恢复上下文 → deepagents 逐事件执行 → on_chat_model_stream 事件
    ↓
提取 token → SSE: data: {"token": "..."}\n\n
    ↓
最后推送 → SSE: data: {"thread_id": "..."}\n\n
    ↓
StreamingResponse(media_type="text/event-stream") 逐条推送
    ↓
前端 fetch + ReadableStream 解析 SSE，保存 thread_id 用于后续对话
```
...
server.py
  ...
  ├── lifespan:
  │     ├── init_db()         ← 自动建表（业务库）
  │     ├── init_graph()      ← 检查点后端 + Agent + 工具 + 文件后端
  │     ├── create_store()    ← Redis / MemoryStore
  │     ├── set_store()       ← 注入依赖
  │     ├── init_jwt()        ← JWT 密钥持久化
  │     └── shutdown_graph()  ← 关闭检查点连接池
  └── api.router
        ├── agent_api.router (prefix="/api")
        │     └── get_graph() → agent.graph (init_graph 管理)
        │           ├── agent.models.llm (DeepSeek)
        │           ├── agent.tools.internet_search (Tavily)
...
| `CHECKPOINT_DB_PATH`         | str | `"./db/langgraph_checkpoints.db"`                     | SQLite 检查点数据库文件路径                  |
| `CHECKPOINT_DB_URL`          | str | `"postgresql://127.0.0.1:5432/ke_hermes"`             | PostgreSQL 检查点数据库连接串               |
...
### 10.11 Agent 图生命周期管理（v1.2.2 新增）

Agent 图从模块级全局变量重构为 `init_graph()` / `get_graph()` / `shutdown_graph()` 生命周期模式。原因：

1. **检查点后端需要异步初始化**：`AsyncSqliteSaver` 需要 `await aiosqlite.connect()`，`AsyncPostgresSaver` 需要 `await pool.open()` + `await checkpointer.setup()`
2. **资源清理**：PostgreSQL 连接池需要在应用关闭时释放（`shutdown_graph()`），防止连接泄漏
3. **FastAPI lifespan 集成**：初始化在 lifespan startup 阶段完成，确保 API 请求到达前检查点已就绪

`get_graph()` 在 API 层懒获取，若未初始化则抛出 `RuntimeError`，防止过早访问。

### 10.12 检查点双后端设计（v1.2.2 新增）

通过 `CHECKPOINT_BACKEND` 环境变量选择 LangGraph 检查点存储后端：

- **SQLite**（默认）：零配置，`aiosqlite` 驱动，适合开发/单机部署
- **PostgreSQL**：`psycopg` 异步驱动 + `AsyncConnectionPool` 连接池，适合生产/多实例部署

两种后端通过 `AsyncSqliteSaver` / `AsyncPostgresSaver` 统一接口接入，`init_graph()` 中的 `if/elif` 分支负责差异处理，Agent 图代码对后端透明。

Windows 平台需要 monkeypatch uvicorn 的 `ProactorEventLoop` → `SelectorEventLoop`，因为 `psycopg` 依赖的 `ProactorEventLoop` 在 Windows 上不支持。

### 10.13 thread_id 上下文管理（v1.2.2 新增）

Chat API 支持 `thread_id` 参数实现多轮对话上下文管理：

- 前端首次对话不传 `thread_id` → 后端通过 `uuid7()` 自动生成（UUID7 按时间排序，优化数据库索引）
- 前端保存返回的 `thread_id`，后续请求携带同一 ID → LangGraph 自动从检查点恢复消息历史
- 流式 SSE 结束时推送 `thread_id`，确保前端及时保存

这一设计使 Agent 具备持久化的"短期记忆"——同一 thread_id 的多次对话共享完整上下文，关闭浏览器后重新打开仍可续接。
...
### 13.3 从 v1.0.0 到 v1.2.2 的变更总结


| 项目       | v1.0.0 (初始)          | v1.2.2 (当前)                                                                 |
| -------- | -------------------- | --------------------------------------------------------------------------- |
| API 模块   | 1 (agent)            | 5 (agent + auth + captcha + oauth + sms)                                    |
| 数据模型     | 0                    | 3 (User + UserOAuth + LoginRecord)                                          |
| Agent 工具 | 0                    | 1 (internet_search / Tavily)                                                |
| 文件后端     | 无                    | FilesystemBackend (workspace/)                                              |
| 中间件      | 0                    | CORS + 依赖注入体系（get_db/get_store/get_client_ip）                               |
| 检查点后端    | 无                    | 双后端（SQLite AsyncSqliteSaver + PostgreSQL AsyncPostgresSaver）                |
| 安全       | 无                    | RSA( cryptography) + JWT(PyJWT + 自动持久化) + bcrypt + 登录锁定                     |
| 存储抽象     | 无                    | KeyValueStore（Redis + MemoryStore 自动降级，含 ttl 方法）                            |
| 对话上下文    | 无                    | thread_id 多轮对话管理（UUID7 + 检查点持久化）                                            |
| 外部依赖     | DeepSeek + DashScope | + bcrypt + cryptography + httpx + Pillow + redis(可选) + tavily + psycopg(可选) |
| 环境变量     | 8                    | 32                                                                          |
| API 接口   | 2                    | 17                                                                          |
...
> 本文档 v1.2.2 基于实际代码实现更新。v1.2.0 设计的全部模块（auth/captcha/oauth/sms）已完成编码并通过功能验证。v1.2.2 重点更新了 Agent 图生命周期管理、检查点双后端（SQLite/PostgreSQL）、thread_id 多轮对话上下文、FilesystemBackend 虚拟文件系统和 internet_search 工具。后续版本应优先实现对话历史 CRUD 和模型列表 API。
