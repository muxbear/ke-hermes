...
本文档为 Ke-Hermes 后端的详细设计说明书 v1.3.0，对照实际代码实现编写。文档覆盖：

- **已实现模块**：智能体框架（含子智能体系统 + 互联网搜索工具 + CompositeBackend 文件后端）、Chat API（普通对话 + SSE 流式 + thread_id 上下文管理）、Conversation API（对话历史 CRUD）、认证授权、验证码、OAuth 第三方登录、短信服务
- **基础设施**：KeyValueStore 存储抽象层（Redis + 内存降级）、RSA + JWT + bcrypt 安全体系、SQLAlchemy 异步 ORM（SQLite + PostgreSQL 双后端）、LangGraph 检查点双后端（SQLite + PostgreSQL）+ LangGraph Store（Memory + PostgreSQL）
...
│   │   └── graph.py                 # deepagents 智能体图定义（生命周期 + CompositeBackend + Store + 子智能体）
│   │
│   ├── api/
│   │   ├── __init__.py              # 导出顶层 router（汇总 6 个子模块）
│   │   ├── deps.py                  # 依赖注入（get_db, get_store, get_client_ip, get_current_user_id）
│   │   ├── agent/
│   │   │   ├── __init__.py          # 导出 agent router
│   │   │   └── agent_api.py         # 智能体 API 路由与数据模型（含 Context 注入 + 对话记录创建）
│   │   ├── auth/
...
│   │   └── models/
│   │       ├── __init__.py          # 导出所有 model（User, UserOAuth, LoginRecord, Conversation）
│   │       ├── user.py              # User 模型
...
│
├── workspace/                       # 智能体工作目录（含 /memories/ 子目录）
│
├── tests/
...
async def lifespan(app: FastAPI):
    ...
    await init_graph()            # 2. 初始化 Agent（检查点 + Store + LLM + 子智能体 + Backend）
...
2. `init_graph()` → 根据 `CHECKPOINT_BACKEND` 创建检查点后端 + LangGraph Store + LLM + 子智能体 + CompositeBackend
...
**职责：** 管理 Agent 图的完整生命周期（初始化 → 运行 → 关闭），支持双检查点后端（SQLite / PostgreSQL）+ 双 Store 后端（InMemory / PostgreSQL），集成子智能体系统和 CompositeBackend。
...
**1. CompositeBackend 替代 FilesystemBackend：**

| 路由               | Backend        | namespace                          | 说明                  |
| ---------------- | -------------- | ---------------------------------- | ------------------- |
| `/memories/`     | `StoreBackend` | `(user_id,)`                       | 用户级记忆隔离，存储在 LangGraph Store |
| default (其他路径)   | `StoreBackend` | `(server_info,)`                   | 服务级别的文件存储           |

- `CompositeBackend` 支持按路径前缀路由到不同的 `StoreBackend`
- `StoreBackend` 将文件数据持久化到 LangGraph Store 中，与检查点共用后端
...
共 **21 个接口**（v1.2.2 为 17 个，v1.3.0 新增 4 个 Conversation 接口）。
...
server.py
  ...
  ├── CORS 中间件
  ├── lifespan:
  │     ├── init_db()         ← 自动建表（业务库，DATABASE_BACKEND 双后端）
  │     ├── init_graph()      ← 检查点 + Store + Agent + 子智能体 + CompositeBackend
  ...
  └── api.router
        ...
        │     │     ├── CompositeBackend (StoreBackend + 路由)
...
### 10.13 CompositeBackend 与多租户隔离（v1.3.0 新增）

`CompositeBackend` 按路径前缀路由到不同的 `StoreBackend`：

- `/memories/` → `StoreBackend(namespace=(user_id,))`：用户级记忆隔离
- default → `StoreBackend(namespace=(server_info,))`：服务级文件存储
...
启动后访问 `http://127.0.0.1:8000/docs`，FastAPI 自动生成交互式 Swagger UI 文档。6 个功能模块共 21 个接口均自动注册。
...
| 功能模块       | 前端 v1.2.1 | 后端 v1.3.0 | 差异                                     |
| ---------- | --------- | --------- | -------------------------------------- |
| 智能体对话 (普通) | ✅ 已实现     | ✅ 已实现     | **一致**（后端已加 JWT 鉴权 + Context + 子智能体委派） |
| 智能体对话 (流式) | ✅ 已实现     | ✅ 已实现     | **一致**                                 |
...
| 模型切换       | ⬜ 已设计     | ⬜         | 前端 ChatHeader 模型选择器已设计，后端无模型列表 API   |
...
### 13.3 从 v1.0.0 到 v1.3.0 的变更总结


| 项目       | v1.0.0 (初始)          | v1.3.0 (当前)                                                                                     |
| -------- | -------------------- | ----------------------------------------------------------------------------------------------- |
| API 模块   | 1 (agent)            | 6 (agent + auth + captcha + oauth + sms + conversation)                                         |
| 数据模型     | 0                    | 4 (User + UserOAuth + LoginRecord + Conversation)                                               |
| Agent 架构 | 单智能体 + 无文件后端         | 主智能体 + 子智能体（research-agent）+ CompositeBackend（StoreBackend + 路由）+ LangGraph Store + Context |
| LLM 模型  | 1 (DeepSeek)         | 2 (DeepSeek 主智能体 + Qwen 3.6 Plus 子智能体)                                                        |
| 文件后端     | 无                    | CompositeBackend（StoreBackend + namespace 多租户隔离）                                              |
| 中间件      | 0                    | CORS + 依赖注入体系（get_db/get_store/get_client_ip/get_current_user_id）                              |
| 检查点后端    | 无                    | 双后端（SQLite + PostgreSQL），含 Store 持久化                                                          |
| 安全       | 无                    | RSA + JWT + bcrypt + 登录锁定 + JWT 鉴权（Chat + Conversation API）                                   |
...
> 本文档 v1.3.0 基于实际代码实现更新。v1.3.0 重点更新了子智能体系统（research-agent + Qwen 3.6 Plus）、CompositeBackend + LangGraph Store 持久化、Context 上下文注入、对话历史 CRUD 模块、双模型架构、业务数据库后端切换（DATABASE_BACKEND）和工作空间配置（WORKSPACE）。后续版本应优先实现模型列表 API 和通知系统。
